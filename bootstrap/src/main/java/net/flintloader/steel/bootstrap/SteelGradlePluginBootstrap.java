...
public class SteelGradlePluginBootstrap implements Plugin<PluginAware> {
	...
	private static final String PLUGIN_CLASS_NAME = "net.flintloader.steel.SteelGradlePlugin";
	...
	public void apply(PluginAware pluginAware) {
		if (pluginAware instanceof Project) {
			...
			if (project.findProperty("flint.steel.skip-env-validation") == null) {
				validateEnvironment();
			} else {
				project.getLogger().lifecycle("Steel environment validation disabled. Please re-enable before reporting any issues.");
			}
		}
	...
	BootstrappedPlugin getActivePlugin() {
		try {
			...
			throw new RuntimeException("Failed to bootstrap steel", e);
		}
	}
}